...
export const getAllProduct = async (search, limit) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`http://localhost:3000/api/product/get-all?filter=name&filter=${search}&limit=${limit}`)
    } else {
...
export const getProductType = async (type) => {
    if (type) {
        const res = await axios.get(`http://localhost:3000/api/product/get-all?filter=type&filter=${type}`)
        return res.data
    }
}
...
export const deleteManyProducts = async (names, access_token) => {
    const res = await axiosJWT.post(`http://localhost:3000/api/product/delete-many`, names, {
        headers: {
            token: `Bearer ${access_token}`,
        }
    })
